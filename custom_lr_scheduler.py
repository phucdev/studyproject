...
def _get_cosine_schedule_with_warmup_embedding_tuning_lr_lambda(
        ...
        embedding_tuning_percentage: int, embedding_tuning_warmup_percentage: int
):
    embedding_tuning_steps = math.ceil(num_training_steps*embedding_tuning_percentage/100)
    embedding_tuning_warmup_steps = math.ceil(embedding_tuning_steps*embedding_tuning_warmup_percentage/100)

    full_training_steps = num_training_steps - embedding_tuning_steps
    full_training_warmup_steps = math.ceil(full_training_steps*warmup_percentage/100)
...
def get_custom_lr_scheduler(optimizer, warmup_percentage, num_training_steps, embedding_tuning_percentage,
                            num_cycles: float = 0.5, min_lr: float = 0.0, embedding_tuning_warmup_percentage: int = 0):
    if embedding_tuning_percentage > 0:
        lr_lambda = partial(
            _get_cosine_schedule_with_warmup_embedding_tuning_lr_lambda,
            warmup_percentage=warmup_percentage,
            num_training_steps=num_training_steps,
            embedding_tuning_percentage=embedding_tuning_percentage,
